...
Using git submodule at work led to multiple problems, often requiring an LLM to
dig out of issues. Could be user error, but it seemed easier to just agentically
code a substitute.

This is not meant to be an exhaustive substitute for git submodule; rather, this is a
focused tool that covers common work patterns, in bash scripts that are easy to
read and fix when something goes wrong.
